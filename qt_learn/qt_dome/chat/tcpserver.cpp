...
void TcpServer::updateClientProgress(qint64 numBytes)//更新进度条
{
   ...
   //num.sprintf("%.1f KB/s", (bytesWritten*1000) / (1024.0*time.elapsed()));
    ...
    {
        ui->serverStatusLabel->setText(QStringLiteral("传送文件 %1 成功").arg(theFileName));
    }
}
...
void TcpServer::on_serverOpenBtn_clicked()
{
    ...
    if(!fileName.isEmpty())
    {
        ...
        ui->serverStatusLabel->setText(QStringLiteral("要传送的文件为：%1 ").arg(theFileName));
...
void TcpServer::initServer()
{
    ...
    ui->serverStatusLabel->setText(QStringLiteral("请选择要传送的文件"));
